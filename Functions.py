...
def combine_data(year):
    ...
    for var_name, dataset in veg_vars.items():
        means, times = extract_vegetation(dataset, var_name, grid_gdf)
    ...
    #elevation
    results["elevation"] = extract_elevation(elev, grid_gdf)
    #Combine everything
...
def extract_vegetation(nc_dataset, var_name, grid_gdf):
    """
...
def extract_elevation(elevation_da, grid_gdf):
    """
